#' Multiple-argument pipe operator
#'
#' \code{lhs \%!>\% rhs} pipes the elements of \code{lhs}
#' into a function or function call (\code{rhs}).
#'
#' The multi-pipe takes a named list on the left hand side and inserts
...
#'  list(b = 2) %!>% (foo(3)(c = 9))
#' @importFrom rlang call2 is_symbol expr expr_deparse call_standardise
#' @importFrom rlang new_quosure get_expr get_env eval_tidy
#' @seealso set_splicr_conflict_handling
`%!>%` =
  function(lhs, rhs) {
    ...
    if(!
       rlang::is_list(lhs)) rlang::abort("lhs must be a list for %!>%")
    ...
    # Create a call with the lhs args
    lhs_call =
      call2(rhs_expr[[1]], !!!lhs)
    # Modify that call to include the rhs args, overwriting named args
    out_call = call_standardise(
    ...
    out = new_quosure(out_call, env = eval_env)
    eval_tidy(out)
  }
